...
namespace common
{
	...
	std::string& OpenSaveFileDialog::getFullPath()
	{ 
		mFullPath = "";
		for (int i = 0; i<mCurrentFolder.length(); i++)
			mFullPath += static_cast<char>(mCurrentFolder.getChar(i));
		mFullPath += "\\";
		for (int i = 0; i<mFileName.length(); i++)
			mFullPath += static_cast<char>(mFileName.getChar(i));
		return mFullPath;
	}
